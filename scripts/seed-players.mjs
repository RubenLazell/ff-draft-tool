...
// Run via: npm run seed:players
//
// Safe to re-run (upserts by id). Only ever upserts, never deletes —
// public.user_rankings.player_id has ON DELETE CASCADE from players,
// so a future "prune stale players" job would silently wipe affected
// users' saved ranks. Don't add deletes here without accounting for that.
...
async function main() {
  ...
  console.log(`Prepared ${rows.length} players to upsert.`);
